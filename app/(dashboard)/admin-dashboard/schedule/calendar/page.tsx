...
export default function CalendarPage() {
  ...
  const handleScheduleSubmit = (data: {
    ...
    // TODO: Add API call to save the schedule
  };
    ...
    return (
      <div className="grid grid-cols-7 gap-2">
        ...
        {weekDays.map((day, index) => {
          ...
          const dateParam = `${day.getFullYear()}-${String(day.getMonth() + 1).padStart(2, '0')}-${String(day.getDate()).padStart(2, '0')}`;

          return (
            <div key={index} className="group relative border rounded-b-md md:p-2 min-h-[200px] max-h-[400px] overflow-y-auto hover:bg-muted/30 transition-colors">
              {/* Add button overlay on hover */}
              <button
                onClick={() => openAddModal(day)}
                className="absolute inset-0 flex items-center justify-center bg-primary/5 opacity-0 group-hover:opacity-100 transition-opacity z-10 cursor-pointer"
              >
          ...
          {weeks.map((week, weekIndex) => (
            <div key={weekIndex} className="grid grid-cols-7 gap-2">
              {week.map((day, dayIndex) => {
                ...
                return (
                  <div key={day} className={`group relative border rounded-md p-0.5 md:p-2 h-24 overflow-y-auto hover:bg-muted/30 transition-colors ${isToday ? "border-blue-300" : ""}`}>
                    {/* Add button overlay on hover */}
                    <button
                      onClick={() => openAddModal(date)}
                      className="absolute inset-0 flex items-center justify-center bg-primary/5 opacity-0 group-hover:opacity-100 transition-opacity z-10 cursor-pointer"
                    >
                      <div className="bg-primary text-primary-foreground rounded-full p-1.5 shadow-lg hover:scale-110 transition-transform">
